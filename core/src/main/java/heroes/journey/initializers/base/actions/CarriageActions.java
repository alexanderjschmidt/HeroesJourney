        ...
        carriage = new TargetAction<>("carriage", "Carriage", "Travel to a kingdom by carriage", null) {
            ...
            protected ActionResult onSelectTarget(TargetInput<Feature> targetInput) {
                UUID town = targetInput.getGameState()
                    .getEntities()
                    .getLocation(targetInput.getInput().location.getX(),
                        targetInput.getInput().location.getY());
                ...
                String locationName = NamedComponent.get(targetInput.getGameState().getWorld(), town,
                    "Unknown Location");
                PositionComponent positionComponent = PositionComponent.get(
                    targetInput.getGameState().getWorld(), targetInput.getEntityId());
